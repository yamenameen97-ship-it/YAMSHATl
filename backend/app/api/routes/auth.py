...
def _requested_frontend_url(request: Request) -> str:
    candidate = str(request.query_params.get('frontend_url') or request.headers.get('origin') or settings.FRONTEND_ORIGIN or '').strip()
    if not candidate.startswith('http'):
        return str(settings.FRONTEND_ORIGIN or '').strip()
    return candidate.rstrip('/')


def _encode_oauth_state(frontend_url: str, provider: str) -> str:
    payload = {
        'frontend_url': frontend_url.rstrip('/'),
        'provider': provider,
        'nonce': secrets.token_urlsafe(12),
    }
    return _urlsafe_b64encode(json.dumps(payload, separators=(',', ':')).encode('utf-8'))


def _decode_oauth_state(state_value: str) -> dict:
    try:
        decoded = _urlsafe_b64decode(state_value)
        return json.loads(decoded.decode('utf-8')) if decoded else {}
    except Exception:
        return {}


def _build_frontend_oauth_redirect(target_frontend: str, *, payload: dict | None = None, error: str = ''):
    target = str(target_frontend or '').strip().rstrip('/')
    if not target or not target.startswith('http'):
        return None
    if error:
        return RedirectResponse(url=f"{target}/#/auth/callback?error={quote(error)}", status_code=status.HTTP_302_FOUND)
    encoded_payload = _urlsafe_b64encode(json.dumps(payload or {}, separators=(',', ':')).encode('utf-8'))
    return RedirectResponse(url=f"{target}/#/auth/callback?payload={quote(encoded_payload)}", status_code=status.HTTP_302_FOUND)


def _oauth_frontend_target(request: Request, state_value: str = '') -> str:
    state_payload = _decode_oauth_state(state_value)
    state_frontend = str(state_payload.get('frontend_url') or '').strip()
    if state_frontend.startswith('http'):
        return state_frontend.rstrip('/')
    return _requested_frontend_url(request)
...
async def google_oauth_login(request: Request):
    ...
    frontend_url = _requested_frontend_url(request)
    state = _encode_oauth_state(frontend_url, 'google') if frontend_url else ''
    return {'url': f'https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id={settings.GOOGLE_OAUTH_CLIENT_ID}&redirect_uri={redirect_uri}&scope=openid%20email%20profile&state={state}'}
...
async def google_oauth_callback(request: Request, response: Response, db: Session = Depends(get_db), code: str = None, state: str = ''):
    ...
    redirect_target = _oauth_frontend_target(request, state)
    redirect_response = _build_frontend_oauth_redirect(redirect_target, payload=session_payload)
    return redirect_response or session_payload
...
async def facebook_oauth_login(request: Request):
    ...
    frontend_url = _requested_frontend_url(request)
    state = _encode_oauth_state(frontend_url, 'facebook') if frontend_url else ''
    return {'url': f'https://www.facebook.com/v18.0/dialog/oauth?client_id={settings.FACEBOOK_OAUTH_CLIENT_ID}&redirect_uri={redirect_uri}&scope=email,public_profile&state={state}'}
...
async def apple_oauth_login(request: Request):
    if not settings.APPLE_CLIENT_ID or not settings.APPLE_OAUTH_REDIRECT_URI:
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail='Apple OAuth not configured')
    frontend_url = _requested_frontend_url(request)
    state = _encode_oauth_state(frontend_url, 'apple') if frontend_url else secrets.token_urlsafe(12)
    return {'url': AppleOAuthService.get_authorization_url(settings.APPLE_OAUTH_REDIRECT_URI, state)}
...
async def apple_oauth_callback(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    code = str(request.query_params.get('code') or form_data.get('code') or '').strip()
    state = str(request.query_params.get('state') or form_data.get('state') or '').strip()
    raw_user = form_data.get('user') if form_data else None

    if not code:
        redirect_target = _oauth_frontend_target(request, state)
        redirect_response = _build_frontend_oauth_redirect(redirect_target, error='Authorization code not provided')
    ...
    tokens = AppleOAuthService.exchange_code_for_tokens(code, settings.APPLE_OAUTH_REDIRECT_URI)
    id_token = str(tokens.get('id_token') or '').strip()
    if not id_token:
        redirect_target = _oauth_frontend_target(request, state)
        redirect_response = _build_frontend_oauth_redirect(redirect_target, error='Apple ID token was not returned')
        if redirect_response:
            return redirect_response
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Apple ID token was not returned')

    user = AppleOAuthService.authenticate_or_register(db, id_token, user_data=user_data)
    binding = request_binding_context(request)
    mark_successful_login(db, user, binding)

(function () {
  ...
  const keepOrigin = (value) => {
    const candidate = trim(value);
    if (!candidate) return false;
    if (isLocalOrigin(candidate)) return currentIsLocal;
    if (!isRenderHost(candidate)) return true;
    if (sameOrigin(candidate)) return true;
    if (deployBackend && candidate === deployBackend) return true;
    if (siblingBackend && candidate === siblingBackend) return true;
    return false;
  };
  ...
  const keepApi = (value) => {
    ...
    if (isLocalOrigin(candidateOrigin)) return currentIsLocal;
    if (!isRenderHost(candidateOrigin)) return true;
    if (sameApiOrigin(candidate)) return true;
    if (deployApi && candidate === deployApi) return true;
    if (siblingApi && candidate === siblingApi) return true;
    return false;
  };

  const safeStoredBackend = keepOrigin(storedBackend) ? storedBackend : '';
  const safeStoredApi = keepApi(storedApi) ? storedApi : '';
  const safeDeployBackend = keepOrigin(deployBackend) ? deployBackend : '';
  const safeDeployApi = keepApi(deployApi) ? deployApi : '';

  const backendOrigin =
    queryBackend ||
    apiToOrigin(queryApi) ||
    safeDeployBackend ||
    apiToOrigin(safeDeployApi) ||
    safeStoredBackend ||

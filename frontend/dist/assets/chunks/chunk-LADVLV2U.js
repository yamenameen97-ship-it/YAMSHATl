...
init_define_import_meta_env();
var ARABIC_DIACRITICS = /[\u064B-\u065F\u0670\u06D6-\u06ED]/g;
var NON_TEXT = /[^\p{L}\p{N}#@._\s-]/gu;
...
function toSearchEntry(type, item = {}) {
  ...
  const route = type === "users" ? `/profile/${encodeURIComponent(username)}` : type === "reels" ? "/reels" : type === "posts" ? "/" : "/search";
  return {
...
function searchInCollections(query = "", collections = {}, options = {}) {
  ...
  return buckets.map((entry) => {
    ...
    const score = normalizedQuery ? fuzzyScore(normalizedQuery, searchable) : 0;
    const popularity = Number(entry.metrics?.likes || 0) * 2 + Number(entry.metrics?.followers || 0) * 3 + Number(entry.metrics?.views || 0) * 0.02 + Number(entry.metrics?.comments || 0) * 2 + Number(entry.metrics?.shares || 0) * 2;
    return {
      ...entry,
      score: Number((score + Math.min(popularity / 1e4, 0.28)).toFixed(4))
    };
  }).filter((entry) => !normalizedQuery || entry.score >= 0.24).sort((a, b) => b.score - a.score);
}
...
async function fetchSuggestedReels(reels = []) {
  try {
    const { data } = await axios_default.get("/recommendations/reels", { params: { limit: 20 } });
...
function explainRecommendation(item = {}) {
  const terms = [item.title, item.name, item.description, ...item.hashtags || []].map((value) => normalizeSearchText(value)).filter(Boolean);

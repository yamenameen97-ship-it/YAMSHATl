...
init_define_import_meta_env();
var DEVICE_ID_KEY = "yamshat_device_id";
var OFFLINE_QUEUE_KEY = "yamshat_offline_queue";
var MAX_RETRY_ATTEMPTS = 3;
var RETRY_DELAY_MS = 1e3;
var OFFLINE_QUEUE_MAX_SIZE = 100;
var VAPID_PUBLIC_KEY = define_import_meta_env_default.VITE_VAPID_PUBLIC_KEY || "";
...
var notificationService = {
  ...
  async registerDevice() {
    ...
    try {
      await retryWithBackoff(() => axios_default.post("/notifications/register-device", payload));
    } catch {
  ...
  async unregisterDevice() {
    try {
      await retryWithBackoff(() => axios_default.post("/notifications/unregister-device", { device_id: getOrCreateDeviceId() }));
    } catch (error) {
  ...
  async subscribeToPushNotifications() {
    ...
    try {
      await axios_default.post("/notifications/subscribe-push", {
        device_id: getOrCreateDeviceId(),
  ...
  async unsubscribePushNotifications() {
    ...
    try {
      await axios_default.post("/notifications/unsubscribe-push", { device_id: getOrCreateDeviceId() });
    } catch {
    }
    return true;
  },
  async fetchNotifications(limit = 50) {
    ...
    try {
      const response = await retryWithBackoff(() => axios_default.get("/notifications", { params: { limit }, cache: true, cacheTtlMs: 2e4 }));
  ...
  async markNotificationRead(notificationId) {
    ...
    try {
      await retryWithBackoff(() => axios_default.post(`/notifications/${encodeURIComponent(notificationId)}/read`));
    } catch (error) {
  ...
  async markAllNotificationsRead() {
    ...
    store.markAllRead();
    try {
      await retryWithBackoff(() => axios_default.put("/notifications/read"));
    } catch (error) {
  ...
  async deleteNotification(notificationId) {
    ...
    try {
      await retryWithBackoff(() => axios_default.delete(`/notifications/${encodeURIComponent(notificationId)}`));
    } catch (error) {
...
var notificationService_default = notificationService;

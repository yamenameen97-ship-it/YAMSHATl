import { z as createUploadSecurityManifest, F as FILE_RULES, af as imageCompression, I as IMAGE_PRESET, i as MEDIA_PROVIDER, M as MEDIA_CDN_BASE, V as VIDEO_PRESET, p as SparkMD5, A as API, h as MEDIA_ENDPOINTS, am as logger, G as defaultRetryManager, aO as resolveMediaUrl, j as MEDIA_SECURITY } from "../index-Dz8FA2T4.js";
...
function validateFile(file) {
  ...
  if (FILE_RULES.allowedMimeTypes.length) {
    ...
    const isAllowed = allowedBase.includes(fileType) || fileType.startsWith("audio/") && allowedBase.some((item) => item === "audio/*" || item.startsWith("audio/"));
    if (!isAllowed) {
...
async function requestWithRetry(fn, { retries = DEFAULT_MAX_CHUNK_RETRIES, onRetry = () => {
}, context = {} } = {}) {
  let attempt = 0;
  while (true) {
    try {
...
async function uploadResumable(file, fingerprint, manifest, onProgress, extraFields = {}, options = {}) {
  ...
  if (!sessionId) {
    try {
      const response2 = await requestWithRetry(() => API.post(MEDIA_ENDPOINTS.resumableStart, {
        filename: file.name,
...
class MediaUploadService {
  ...
  async uploadFile(file, options = {}) {
    ...
    const upload = requiresResumable ? await uploadResumable(prepared.file, fingerprint, prepared.manifest, onProgress, extraFields, {
      signal,
  ...
  async uploadVoiceNote(blob, options = {}) {
    ...
    const extByType = rawType.includes("ogg") ? "ogg" : rawType.includes("mpeg") ? "mp3" : rawType.includes("mp4") || rawType.includes("m4a") ? "m4a" : rawType.includes("wav") ? "wav" : "webm";
    const fileName = options?.fileName || `voice-note-${Date.now()}.${extByType}`;
    const file = blob instanceof File ? new File([blob], blob.name || fileName, { type: rawType, lastModified: blob.lastModified || Date.now() }) : new File([blob], fileName, { type: rawType });
...
const uploadFile = (file, optionsOrOnProgress) => {
  const options = typeof optionsOrOnProgress === "function" ? { onProgress: optionsOrOnProgress } : optionsOrOnProgress;
  return mediaUploadService.uploadFile(file, options);
};
export {
  mediaUploadService as m,
  uploadFile as u
};

...
function buildAdaptiveSource(reel = {}, quality = "high") {
...
function computeAutoQuality({ manualQuality = "auto", preferredQuality = "high", bufferEvents = 0, saveData = false, effectiveType = "4g" } = {}) {
  if (manualQuality && manualQuality !== "auto") return manualQuality;
  let base = saveData || effectiveType === "2g" ? "low" : effectiveType === "3g" ? "medium" : preferredQuality;
...
function saveWatchHistoryEntry(entry = {}) {
  ...
  const normalized = {
    ...
    watchedAt: entry.watchedAt || (/* @__PURE__ */ new Date()).toISOString(),
...
function trackReelAnalytics(eventType, payload = {}) {
...
function submitModerationReport(report = {}) {
...
function formatWatchPercentage(progress = 0) {
  return `${Math.round(Number(progress || 0) * 100)}%`;
}
export {
  getModerationReports as a,
  buildAdaptiveSource as b,
  computeAutoQuality as c,
  getReelInsightsById as d,
  getReelsAnalyticsDashboard as e,
  formatWatchPercentage as f,
  getDeviceProfile as g,
  getReelsCache as h,
  getWatchHistory as i,
  primeVideo as j,
  saveWatchHistoryEntry as k,
  submitModerationReport as l,
  preloadPoster as p,
  saveReelsCache as s,
  trackReelAnalytics as t
};

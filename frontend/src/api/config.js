...
const safeRuntimeBackendOrigin = keepOrigin(runtimeBackendOrigin) ? runtimeBackendOrigin : '';
const safeRuntimeApi = keepApi(runtimeApi) ? runtimeApi : '';
const safeEnvBackendOrigin = keepOrigin(envBackendOrigin) ? envBackendOrigin : '';
const safeEnvApi = keepApi(envApi) ? envApi : '';
const safeEnvSocketUrl = keepOrigin(envSocketUrl) ? envSocketUrl : '';
...
export const BACKEND_ORIGIN = trim(
  queryBackend ||
    ...
    safeEnvBackendOrigin ||
    safeStoredBackend ||
    apiToOrigin(safeRuntimeApi || safeEnvApi || safeStoredApi) ||
    siblingBackend ||
    currentOrigin
);

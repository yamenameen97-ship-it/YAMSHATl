...
const rejectInvalidRemoteHost = (value) =>
  isLocalNetworkHost(value) && !isLocalNetworkHost(currentOrigin);
...
const sanitizeApiCandidate = (value) => {
  ...
  return rejectInvalidRemoteHost(apiToOrigin(normalized))
    ? ''
    : normalized;
};
...
const renderOriginMismatch = (candidate, expected) => {
  ...
  if (
    !isRenderHost(normalizedCandidate) ||
    !isRenderHost(normalizedExpected)
  ) {
    return false;
  }
...
const renderApiMismatch = (candidate, expected) =>
  renderOriginMismatch(apiToOrigin(candidate), apiToOrigin(expected));
...
const storedBackend = sanitizeOriginCandidate(
  readStored('backendOrigin')
);

const runtimeApi = sanitizeApiCandidate(
  window.APP_API_BASE || ''
);

const runtimeBackendOrigin = sanitizeOriginCandidate(
  window.YAMSHAT_BACKEND_ORIGIN ||
    window.APP_BACKEND_ORIGIN
);

const envApi = sanitizeApiCandidate(
  import.meta.env.VITE_API_BASE || ''
);

const envBackendOrigin = sanitizeOriginCandidate(
...
const envSocketUrl = sanitizeOriginCandidate(
  import.meta.env.VITE_SOCKET_URL ||
    envBackendOrigin
);

const envCdnBase = trim(
  import.meta.env.VITE_CDN_BASE || ''
);

const inferredBackendOrigin = inferBackendOrigin();

const inferredApi = inferredBackendOrigin
  ? `${inferredBackendOrigin}/api`
  : '';
...
const originLooksCurrent = (value) => {
  ...
  if (!candidate || !inferredBackendOrigin) {
    return false;
  }

  return (
    candidate === inferredBackendOrigin ||
...
const apiLooksCurrent = (value) => {
  ...
  return (
    candidate === toApiBase(inferredApi) ||
    candidate === toApiBase(`${currentOrigin}/api`)
  );
};

const storedBackendConflictsWithEnv =
  renderOriginMismatch(
    storedBackend,
    envBackendOrigin
  );

const storedApiConflictsWithEnv =
  renderApiMismatch(
    storedApi,
    envApi
  );
...
const safeStoredBackend =
  (
    originLooksCurrent(storedBackend) ||
    !isRenderHost(storedBackend)
  ) &&
  !storedBackendConflictsWithEnv
    ? storedBackend
    : '';

const safeStoredApi =
  (
    apiLooksCurrent(storedApi) ||
    !isRenderHost(apiToOrigin(storedApi))
  ) &&
  !storedApiConflictsWithEnv
    ? storedApi
    : '';

const runtimeBackendIsFrontendOrigin =
  Boolean(
    runtimeBackendOrigin &&
      runtimeBackendOrigin === currentOrigin &&
      inferredBackendOrigin !== currentOrigin
  );

const runtimeApiIsFrontendOrigin =
  Boolean(
    runtimeApi &&
      runtimeApi ===
...
const runtimeBackendConflictsWithEnv =
  renderOriginMismatch(
    runtimeBackendOrigin,
    envBackendOrigin
  );

const runtimeApiConflictsWithEnv =
  renderApiMismatch(
    runtimeApi,
    envApi
  );

const safeRuntimeBackendOrigin =
  runtimeBackendIsFrontendOrigin ||
  runtimeBackendConflictsWithEnv
    ? ''
    : runtimeBackendOrigin;

const runtimeApiMatchesRuntimeBackend =
  Boolean(
    !safeRuntimeBackendOrigin ||
      !runtimeApi ||
      apiToOrigin(runtimeApi) ===
        safeRuntimeBackendOrigin
  );

const safeRuntimeApi =
  runtimeApiIsFrontendOrigin ||
  !runtimeApiMatchesRuntimeBackend ||
  runtimeApiConflictsWithEnv
    ? ''
    : runtimeApi;

const queryBackendApi = queryBackend
  ? `${queryBackend}/api`
  : '';

const runtimeBackendApi =
  safeRuntimeBackendOrigin
    ? `${safeRuntimeBackendOrigin}/api`
    : '';
...
export const CDN_BASE = trim(
  window.YAMSHAT_CDN_BASE ||
    window.APP_CDN_BASE ||
    envCdnBase ||
    ''
);
...
const fallbackSocketOrigin =
  rejectInvalidRemoteHost(BACKEND_ORIGIN)
    ? currentOrigin
    : BACKEND_ORIGIN;
...
export const SOCKET_URL = trim(
  queryBackend ||
    ...
    envSocketUrl ||
    safeRuntimeBackendOrigin ||
    sanitizeOriginCandidate(
      window.YAMSHAT_SOCKET_URL || ''
    ) ||
    safeStoredBackend ||
    fallbackSocketOrigin ||
    currentOrigin
);

try {
  const previousBackendOrigin = trim(
    localStorage.getItem('backendOrigin')
  );

  const backendOriginChanged = Boolean(

...
export const SecurityDevicesUI = () => {
  ...
  const fetchSecurityData = useCallback(async () => {
    try {
      ...
      const devicesResponse = await axios.get(`${API_BASE_URL}/security/trusted-devices`);
      setTrustedDevices(devicesResponse.data);

      // جلب الجلسات النشطة
      const sessionsResponse = await axios.get(`${API_BASE_URL}/security/active-sessions`);
      setActiveSessions(sessionsResponse.data);

      // جلب تسجيلات الدخول المريبة
      const loginsResponse = await axios.get(`${API_BASE_URL}/security/suspicious-logins`);
  ...
  const removeDevice = useCallback(async (deviceId) => {
    try {
      await axios.delete(`${API_BASE_URL}/security/trusted-devices/${deviceId}`);
  ...
  const terminateSession = useCallback(async (sessionId) => {
    try {
      await axios.post(`${API_BASE_URL}/security/sessions/${sessionId}/terminate`);
  ...
  const confirmSuspiciousLogin = useCallback(async (loginId) => {
    try {
      await axios.post(`${API_BASE_URL}/security/suspicious-logins/${loginId}/confirm`);
  ...
  const denySuspiciousLogin = useCallback(async (loginId) => {
    try {
      await axios.post(`${API_BASE_URL}/security/suspicious-logins/${loginId}/deny`);
      setSuspiciousLogins(prev => prev.filter(l => l.id !== loginId));
    } catch (error) {

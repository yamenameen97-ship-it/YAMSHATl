...
function Avatar({ username }) {
  ...
  return <div className="yam-inbox-avatar">{initial}</div>;
}
...
export default function Inbox() {
  ...
  return (
    <MainLayout>
      <section className="yam-inbox-page" dir="rtl">
        <div className="yam-inbox-shell">
          <header className="yam-inbox-header">
            <div>
              <h1>الدردشات</h1>
              <p>واجهة مرتبة مثل قائمة المحادثات: اسم، آخر رسالة، والوقت بدون فوضى.</p>
            </div>
            <div className="yam-inbox-stats">{filteredThreads.length} محادثة</div>
          </header>

          <div className="yam-inbox-toolbar">
            <div className="yam-inbox-tabs">
              <button type="button" className={activeTab === 'all' ? 'active' : ''} onClick={() => setActiveTab('all')}>الكل</button>
              <button type="button" className={activeTab === 'pinned' ? 'active' : ''} onClick={() => setActiveTab('pinned')}>المثبتة</button>
              <button type="button" className={activeTab === 'archived' ? 'active' : ''} onClick={() => setActiveTab('archived')}>المؤرشفة</button>
            </div>
            <input
              type="search"
          ...
          <div className="yam-inbox-list">
            {isLoading ? <div className="yam-inbox-empty">جارٍ تحميل المحادثات...</div> : null}
            {!isLoading && !filteredThreads.length ? <div className="yam-inbox-empty">لا توجد محادثات مطابقة حالياً.</div> : null}
            ...
            {filteredThreads.map((thread) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="yam-thread-body">
                    <div className="yam-thread-row-top">
                      <strong>{username}</strong>
                      <span>{lastTime}</span>
                    </div>

                    <div className="yam-thread-row-bottom">
                      <p>{thread.last_message || 'ابدأ المحادثة الآن'}</p>
                      <div className="yam-thread-meta-tools">
                        {isMuted ? <span className="yam-mini-flag">🔕</span> : null}
                        {isPinned ? <span className="yam-mini-flag">📌</span> : null}
                        {thread.unread_count > 0 ? <span className="yam-unread-pill">{thread.unread_count}</span> : null}
                      </div>
                    </div>
                  </div>

                  <div className="yam-thread-tools" onClick={(event) => event.stopPropagation()}>
                    <button type="button" title="تثبيت" onClick={(event) => toggleSetMembership(setPinnedChats, pinnedChats, username, event)}>
                      {isPinned ? '📍' : '📌'}
                    </button>
                    <button type="button" title="كتم" onClick={(event) => toggleSetMembership(setMutedChats, mutedChats, username, event)}>
                      {isMuted ? '🔊' : '🔇'}
                    </button>
                    <button type="button" title="أرشفة" onClick={(event) => toggleSetMembership(setArchivedChats, archivedChats, username, event)}>
                      {archivedChats.has(username) ? '📤' : '🗂️'}
                    </button>
                  </div>
                </button>
              );

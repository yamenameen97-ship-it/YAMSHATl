...
export default function LoginEnhanced() {
  ...
  const handleSubmit = async (event) => {
    ...
    const captchaSnapshot = captcha.captcha_id;
    ...
    try {
      ...
      const shouldForceRefreshCaptcha = captchaRelated || statusCode === 429 || statusCode >= 500;
      if (shouldForceRefreshCaptcha) {
        setForm((prev) => ({ ...prev, captchaAnswer: '' }));
        await loadCaptcha(true);
      }
    } finally {
      ...
      <form className="auth-form auth-form-enhanced" onSubmit={handleSubmit} noValidate>
        ...
        {error && (
          <div className="alert error" style={{ display: 'flex', alignItems: 'center', gap: 10, animation: 'shake 0.4s ease' }}>
            ...
            {retryCount > 1 && (
              <button type="button" onClick={handleSubmit} disabled={loading || captchaLoading} style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', textDecoration: 'underline', fontSize: 12 }}>
                إعادة المحاولة
              </button>
            )}
          </div>

...
/**
 * CloseFriendsManagerPage — v87.11
 * صفحة إدارة قائمة الأصدقاء المقربين (Close Friends).
 * القصص المنشورة بخصوصية "المقربون" لا تظهر إلا لهذه القائمة.
 */
export default function CloseFriendsManagerPage() {
  ...
  return (
    ...
    >
      <div dir="rtl" style={{ fontFamily: "'Noto Sans Arabic', 'Tajawal', system-ui, sans-serif" }}>
        <SettingsSection title="الأعضاء" description={`العدد الحالي: ${list.length}`}>
          <div className="cfm-toolbar">
            <button type="button" className="cfm-add-btn" onClick={() => setPickerOpen(true)}>
              <span aria-hidden>＋</span> إضافة صديق مقرّب
            </button>
            <button type="button" className="cfm-refresh" onClick={load} disabled={loading}>
              {loading ? '…' : 'تحديث'}
            </button>
          </div>

          {error ? <div className="cfm-empty cfm-error">{error}</div> : null}

          {loading ? (
            <div className="cfm-empty">جارٍ التحميل…</div>
          ) : list.length === 0 ? (
            <div className="cfm-empty">
              لا يوجد أصدقاء مقربون بعد.<br/>
              اضغط «إضافة صديق مقرّب» لبناء قائمتك.
            </div>
          ) : (
            <div className="cfm-list">
              {list.map((u) => (
                <div key={u.username} className="cfm-row">
                  <div className="cfm-user">
                    <img
                    ...
                    />
                    <div>
                      <div className="cfm-name">@{u.username}</div>
                      {u.created_at ? (
                        <div className="cfm-since">
                          مضاف منذ {new Date(u.created_at).toLocaleDateString('ar-EG')}
                        </div>
                      ) : null}
                    </div>
                  </div>

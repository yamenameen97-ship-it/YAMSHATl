...
===================
نقطة الدخول الموحدة لخدمة call-service.
يستورد التطبيق من الموديول الموجود مسبقاً (enhanced_call_service.py) ويوفر:
  - /health   : فحص الحالة
  - /         : معلومات أساسية
  - CORS صحيح
...
logger = logging.getLogger("yamshat.call-service")

# محاولة استيراد app من الموديول الموجود — إن لم نجده، نُنشئ FastAPI افتراضي
try:
    from enhanced_call_service import app as inner_app  # type: ignore
    app = inner_app
    logger.info("[call-service] ✅ تم استيراد app من enhanced_call_service")
except Exception as exc:  # noqa: BLE001
    logger.warning(f"[call-service] ⚠️  لم يُستورد enhanced_call_service ({exc}) — أُنشئ FastAPI افتراضي")
...
async def _root():
    return {"service": "call-service", "status": "ok"}
...
async def _unhandled(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={"detail": "Internal server error", "service": "call-service"},
        headers={

...
import com.socialapp.navigation.AppNavigator
import com.socialapp.network.SessionManager
import com.socialapp.viewmodels.MainViewModel
...
class MainActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Apply security protections (screenshot, etc)
        com.socialapp.utils.AppSecurityManager.applyScreenProtection(this)
        ...
        MonitoringManager.trackScreen("main")
        PerformanceMonitor.startFrameCollection(this)
        
        // Setup Responsive UI for notches and edge-to-edge
        com.socialapp.utils.ResponsiveUtils.setupEdgeToEdge(this, binding.root)
        ...
        // Check for Root on Startup
        if (com.socialapp.utils.AppSecurityManager.isDeviceRooted() && !BuildConfig.DEBUG) {
            showError("لا يمكن تشغيل التطبيق على أجهزة معدلة (Rooted)") { finish() }
        }
    }

    private fun setupObservers() {
        // Observe login state, navigation targets, etc.
    }

    private fun handleIntent(intent: Intent) {
        val data: Uri? = intent.data
        if (data != null) {
            navigator.handleDeepLink(data)
        } else {
    ...
    override fun onDestroy() {
        ...
        fileChooserCallback = null
        if (::binding.isInitialized) {
            val (totalFrames, slowFrames) = PerformanceMonitor.stopFrameCollection(this)
            MonitoringManager.trackMetric("main_total_frames", totalFrames.toLong(), "frames")
            MonitoringManager.trackMetric("main_slow_frames", slowFrames.toLong(), "frames")
        ...
        binding.webView.apply {
            ...
            webViewClient = object : WebViewClient() {
                ...
                override fun onPageFinished(view: WebView?, url: String?) {
                    ...
                    MonitoringManager.sampleRuntimeHealth(this@MainActivity, "web_page_finished")
                }
            }
        }
    }
    ...
    private fun setupBottomNavigation() {
        binding.bottomNav.setOnItemSelectedListener { item ->
            ...
            if (normalizeTargetPath(target) != currentTargetPath) {
                MonitoringManager.trackNavigation(target)
